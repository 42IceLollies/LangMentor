...
authRouter.get("/", (req, res) => {
  res.json({
    status: 200,
    message: "Welcome to the auth ReST API for LangMentor",
  });
});
...
authRouter.post("/register", async (req, res) => {
  ...
  if (
    utils.validateFields([username, email, password]) &&
    utils.emailValid(email)
  ) {
    ...
    if (userExists) {
      res.json({
        status: 409,
        message:
          "That user already exists. Please register with a different username or email",
      });
    } else {
      try {
        const hashed = await bcrypt.hash(password, 10);
        await pool.query(
          "INSERT INTO unverified_users (user_name, user_email, user_pass) VALUES ($1, $2, $3)",
          [username, email, hashed]
        );
        const accessToken = jwt.sign(
          { username, email, pass: hashed },
        ...
        sendVerificationEmail(email, username, hashed, (response) => {
          if (response) res.json({ status: 200, accessToken, refreshToken });
            ...
            res.json({
              status: 500,
              message:
                "Sorry, something went wrong sending the verificaiton email...",
            });
        });
      } catch (err) {
        console.log(err);
  ...
  if (
    ...
    if (userExists) {
      try {
        ...
        if (validPass) {
          const accessToken = jwt.sign(
            { username, email, pass: userExists.user_pass },
            process.env.ACCESS_TOKEN_SECRET,
            { expiresIn: 60 }
          );
          const refreshToken = jwt.sign(
...
authRouter.get("/verify/:_name/:_email/:_pass", async (req, res) => {
  ...
  const [username, email, password] = encodeUtils.decodeStrings([
    _name,
    _email,
    _pass,
  ]);
  ...
  if (userExists) {
    if (password === userExists.user_pass) {
      try {
        ...
        await pool.query(
          "INSERT INTO users (user_name, user_email, user_pass) VALUES ($1, $2, $3)",
          [user_name, user_email, user_pass]
        );
        await pool.query(
          "DELETE FROM unverified_users WHERE user_name = $1 AND user_email = $2 AND user_pass = $3",
          [user_name, user_email, user_pass]
        );
        res.json({ status: 200 });
      } catch (err) {
      ...
      res.json({
        status: 403,
        message: "Invalid password in verification link",
      });
